...
@section('title', __('Create Account') . ' - YeaBneh Store')
...
<section class="py-16 min-h-[70vh] flex items-center">
    <div class="max-w-[460px] mx-auto px-5 w-full">
        <div class="text-center mb-10">
            ...
            <h1 class="text-2xl lg:text-3xl font-extrabold uppercase tracking-[-0.02em] mb-2">{{ __('Create Account') }}</h1>
            <p class="text-gray-500 text-[14px]">{{ __('Join YeaBneh Store and start training') }}</p>
        </div>
        ...
        <div class="space-y-3 mb-8">
            <a href="{{ route('auth.provider', 'google') }}" class="flex items-center gap-3 w-full border border-gray-200 px-5 py-3.5 rounded-xl hover:border-gray-400 transition-colors group">
                ...
                <span class="text-[14px] font-medium text-gray-700 group-hover:text-black">{{ __('Continue with Google') }}</span>
            </a>
            ...
            <a href="{{ route('auth.provider', 'microsoft') }}" class="flex items-center gap-3 w-full border border-gray-200 px-5 py-3.5 rounded-xl hover:border-gray-400 transition-colors group">
                ...
                <span class="text-[14px] font-medium text-gray-700 group-hover:text-black">{{ __('Continue with Microsoft') }}</span>
            </a>
            ...
            <a href="{{ route('auth.provider', 'github') }}" class="flex items-center gap-3 w-full border border-gray-200 px-5 py-3.5 rounded-xl hover:border-gray-400 transition-colors group">
                ...
                <span class="text-[14px] font-medium text-gray-700 group-hover:text-black">{{ __('Continue with GitHub') }}</span>
            </a>
        </div>

        <div class="relative mb-8">
            <div class="h-px bg-gray-200"></div>
            <span class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white px-4 text-[12px] text-gray-400 font-medium uppercase tracking-wider">{{ __('or sign up with email') }}</span>
        </div>
        ...
        <form method="POST" action="{{ route('register.post') }}">
            ...
            <div class="space-y-5">
                <div>
                    ...
                    @error('name') <p class="text-red-500 text-[12px] mt-1.5">{{ $message }}</p> @enderror
                </div>
                <div>
                ...
                <div>
                    <label class="block text-[11px] font-bold uppercase tracking-[0.15em] text-gray-500 mb-2">{{ __('Password') }}</label>
...
{{ __('Create Account') }}
        ...
        <p class="text-center mt-8 text-[13px] text-gray-500">
            ...
            <a href="{{ route('login') }}" class="text-brand font-semibold hover:underline">{{ __('Sign In') }}</a>
        </p>
    </div>
</section>
