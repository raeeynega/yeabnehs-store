...
@section('title', __('Checkout') . ' - YeaBneh Store')

@section('content')

<section class="py-16">
    <div class="max-w-[1680px] mx-auto px-5 lg:px-10">
        <h1 class="text-3xl lg:text-4xl font-extrabold uppercase tracking-[-0.02em] mb-10">{{ __('Checkout') }}</h1>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="border border-gray-100 p-7 rounded-xl">
                        <h2 class="text-[13px] font-bold uppercase tracking-[0.15em] mb-6">{{ __('Contact Information') }}</h2>
                    ...
                    <!-- Shipping -->
                    <div class="border border-gray-100 p-7 rounded-xl">
                        <h2 class="text-[13px] font-bold uppercase tracking-[0.15em] mb-6">{{ __('Shipping Address') }}</h2>
                        <label class="block text-[11px] font-bold uppercase tracking-[0.15em] text-gray-500 mb-2">{{ __('Address') }} *</label>
                    ...
                    <!-- Notes -->
                    <div class="border border-gray-100 p-7 rounded-xl">
                        <label class="block text-[11px] font-bold uppercase tracking-[0.15em] text-gray-500 mb-2">{{ __('Order Notes (Optional)') }}</label>
                        <textarea name="notes" rows="2"
                            class="w-full border border-gray-200 px-4 py-3.5 text-[14px] focus:outline-none focus:border-brand transition-colors rounded-lg placeholder:text-gray-400" placeholder="{{ __('Any special requests...') }}">{{ old('notes') }}</textarea>
                    </div>

                    <!-- Payment Info -->
                    <div class="border border-gray-100 p-7 rounded-xl">
                        <h2 class="text-[13px] font-bold uppercase tracking-[0.15em] mb-4">{{ __('Payment') }}</h2>
                        <div class="bg-surface-muted p-5 rounded-lg flex items-start gap-4">
                            ...
                            <div>
                                <p class="text-[13px] font-semibold mb-1">{{ __('Pay via CBE or Telebirr') }}</p>
                                <p class="text-[12px] text-gray-500 leading-relaxed">{{ __('After placing your order, you will be able to choose between bank transfer via Commercial Bank of Ethiopia (CBE) or mobile payment via Telebirr. You will receive complete payment instructions.') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="lg:col-span-1">
                    <div class="bg-surface-muted p-7 rounded-xl sticky top-24">
                        <h2 class="text-[13px] font-bold uppercase tracking-[0.15em] mb-6">{{ __('Your Order') }}</h2>
                        ...
                        <div class="space-y-2.5 text-[14px] mt-4">
                            ...
                            <div class="flex justify-between">
                                ...
                                <span class="font-bold">{{ $shipping == 0 ? __('Free') : '$' . number_format($shipping, 2) }}</span>
                            </div>
