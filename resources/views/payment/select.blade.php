...
<section class="py-16 min-h-[70vh] flex items-center">
    <div class="max-w-[800px] mx-auto px-5 w-full">
        <div class="text-center mb-10">
            ...
            <h1 class="text-3xl lg:text-4xl font-extrabold uppercase tracking-[-0.02em] mb-3">ክፍያ ይምረጡ</h1>
            <p class="text-gray-500 text-[14px]">ትዕዛዝ <strong>{{ $order->order_number }}</strong> &mdash; ጠቅላላ: <strong>${{ number_format($order->total, 2) }}</strong></p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <form action="{{ route('payment.instructions', $order->order_number) }}" method="POST">
                ...
                <button type="submit" class="w-full group border-2 border-gray-200 hover:border-brand p-8 text-left transition-all duration-300 hover:shadow-lg">
                    ...
                    <p class="text-[13px] text-gray-500 mb-4 leading-relaxed">በኛው CBE የባንክ መለያ ላይ ቀጥታ ዘውውሩ። የመለያ ቁጥርና መመሪያዎች በቀጣዩ እርምጃ ይረከባሉ።</p>
                    <div class="flex items-center gap-2 text-[12px] font-semibold text-brand group-hover:text-accent-dark transition-colors">
                        <span>ከ CBE ጋር ይቀጥሉ</span>
            ...
            <form action="{{ route('payment.instructions', $order->order_number) }}" method="POST">
                ...
                <button type="submit" class="w-full group border-2 border-gray-200 hover:border-brand p-8 text-left transition-all duration-300 hover:shadow-lg">
                    ...
                    <p class="text-[13px] text-gray-500 mb-4 leading-relaxed">በ Telebirr ሞባይል ገንዘብ ይክፈሉ። በስልክ ቁጥርዎ ወደ እኛ አከራካሪ መለያ ገንዘብ ይልኩ።</p>
                    <div class="flex items-center gap-2 text-[12px] font-semibold text-brand group-hover:text-accent-dark transition-colors">
                        <span>ከ Telebirr ጋር ይቀጥሉ</span>
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                    </div>
                </button>
            </form>
