...
@section('title', 'Payment Pending - YeaBneh Store')
...
<section class="py-16 min-h-[70vh] flex items-center">
    <div class="max-w-[700px] mx-auto px-5 w-full">
        <div class="text-center mb-10">
            ...
            <h1 class="text-3xl lg:text-4xl font-extrabold uppercase tracking-[-0.02em] mb-3">Payment Under Review</h1>
            <p class="text-gray-500 text-[14px]">We've received your payment confirmation</p>
        </div>
        ...
        <div class="border border-gray-100 p-8 mb-8">
            <div class="grid grid-cols-2 gap-y-4 gap-x-8">
                <div class="col-span-2 flex justify-between items-center pb-4 border-b border-gray-100">
                    <span class="text-[12px] font-bold uppercase tracking-[0.12em] text-gray-400">Payment Number</span>
                    <span class="text-[14px] font-semibold">{{ $payment->payment_number }}</span>
                </div>
                <div class="py-3 border-b border-gray-100">
                    <span class="block text-[11px] font-bold uppercase tracking-[0.12em] text-gray-400 mb-1">Order</span>
                    <span class="text-[14px] font-semibold">{{ $order->order_number }}</span>
                </div>
                <div class="py-3 border-b border-gray-100">
                    <span class="block text-[11px] font-bold uppercase tracking-[0.12em] text-gray-400 mb-1">Method</span>
                    <span class="text-[14px] font-semibold">{{ $payment->methodName() }}</span>
                </div>
                <div class="py-3 border-b border-gray-100">
                    <span class="block text-[11px] font-bold uppercase tracking-[0.12em] text-gray-400 mb-1">Amount</span>
                    <span class="text-[14px] font-bold">${{ number_format($payment->amount, 2) }}</span>
                </div>
                <div class="py-3 border-b border-gray-100">
                    <span class="block text-[11px] font-bold uppercase tracking-[0.12em] text-gray-400 mb-1">Status</span>
                ...
                <div class="py-3 border-b border-gray-100">
                    <span class="block text-[11px] font-bold uppercase tracking-[0.12em] text-gray-400 mb-1">{{ $payment->method === 'cbe' ? 'Reference Number' : 'Tx ID' }}</span>
                    <span class="text-[14px] font-semibold">{{ $payment->transaction_ref }}</span>
                </div>
                @endif
                <div class="py-3">
                    <span class="block text-[11px] font-bold uppercase tracking-[0.12em] text-gray-400 mb-1">Submitted</span>
        ...
        <div class="bg-blue-50 border border-blue-200 p-6 mb-8">
            <div class="flex items-start gap-3">
                ...
                <div>
                    <p class="text-[13px] font-semibold text-blue-800 mb-1">What happens next?</p>
                    <p class="text-[12px] text-blue-700 leading-relaxed">Our team will verify your payment within <strong>1-2 business hours</strong>. You'll receive an email at <strong>{{ $order->customer_email }}</strong> once your payment is confirmed and your order is being processed.</p>
                </div>
            </div>
        </div>
