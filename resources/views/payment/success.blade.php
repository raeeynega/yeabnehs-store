...
@section('title', 'Payment Confirmed - YeaBneh Store')
...
<section class="py-16 min-h-[70vh] flex items-center">
    <div class="max-w-[700px] mx-auto px-5 w-full">
        <div class="text-center mb-10">
            ...
            <h1 class="text-3xl lg:text-4xl font-extrabold uppercase tracking-[-0.02em] mb-3">Order Confirmed</h1>
            <p class="text-gray-500 text-[14px]">Thank you for your purchase! Your order is being processed.</p>
        </div>
        ...
        <div class="border border-gray-100 p-8 mb-8">
            <div class="grid grid-cols-2 gap-y-4 gap-x-8 mb-6 pb-6 border-b border-gray-100">
                <div>
                    <span class="block text-[11px] font-bold uppercase tracking-[0.12em] text-gray-400 mb-1">Order Number</span>
                    <span class="text-[16px] font-extrabold">{{ $order->order_number }}</span>
                </div>
                <div>
                    <span class="block text-[11px] font-bold uppercase tracking-[0.12em] text-gray-400 mb-1">Total</span>
                    <span class="text-[16px] font-extrabold">${{ number_format($order->total, 2) }}</span>
                </div>
                <div>
                    <span class="block text-[11px] font-bold uppercase tracking-[0.12em] text-gray-400 mb-1">Payment Method</span>
                    <span class="text-[14px] font-semibold">{{ $payment->methodName() }}</span>
                </div>
                <div>
                    <span class="block text-[11px] font-bold uppercase tracking-[0.12em] text-gray-400 mb-1">Payment Status</span>
                    <span class="inline-block px-3 py-1 text-[11px] font-bold uppercase tracking-wider {{ $payment->statusColor() }}">{{ $payment->status }}</span>
                </div>
            </div>

            <h3 class="text-[12px] font-bold uppercase tracking-[0.15em] mb-4">Items</h3>
            <div class="space-y-4">
                @foreach($order->items as $item)
                    <div class="flex gap-4 items-center">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-[12px] text-gray-400">Qty: {{ $item->quantity }}</p>
                        </div>
            ...
            <div class="mt-6 pt-6 border-t border-gray-100">
                <h3 class="text-[12px] font-bold uppercase tracking-[0.15em] mb-3">Shipping To</h3>
        ...
        <div class="bg-blue-50 border border-blue-200 p-6 mb-8">
            <div class="flex items-start gap-3">
                ...
                <div>
                    <p class="text-[13px] font-semibold text-blue-800 mb-1">What's Next?</p>
                    <p class="text-[12px] text-blue-700 leading-relaxed">
                        ...
                        Your order will be shipped within 2-5 business days after payment confirmation.
                    </p>
                </div>
            </div>
        </div>
