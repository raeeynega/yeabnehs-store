...
<head>
    ...
    <title>@yield('title', 'Security Dashboard') - YeaBneh</title>
...
<body class="bg-gray-50 text-gray-900 font-sans antialiased min-h-screen flex">
    ...
    <aside class="w-64 bg-[#0a0a0a] text-white min-h-screen fixed left-0 top-0 bottom-0 z-40 flex flex-col">
        <div class="p-6 border-b border-white/10">
            ...
            <span class="text-[10px] font-bold uppercase tracking-[0.2em] text-white/50">Security Dashboard</span>
        </div>
        ...
        <div class="p-4 border-t border-white/10">
            ...
            <a href="{{ route('account') }}" class="block px-4 py-2 text-[11px] text-gray-400 hover:text-white transition">&larr; Back to Store</a>
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button class="block w-full text-left px-4 py-2 text-[11px] text-gray-400 hover:text-white transition">Sign Out</button>
            </form>
        </div>
    </aside>
    ...
    <div class="flex-1 ml-64">
        ...
        <header class="bg-white border-b border-gray-200 px-8 py-4 sticky top-0 z-30">
            <div class="flex items-center justify-between">
                <h1 class="text-lg font-bold">@yield('header', 'Security Dashboard')</h1>
                <div class="flex items-center gap-4">
                    <div class="text-right">
                        <p class="text-[11px] text-gray-400">System Status</p>
                        <p class="text-[12px] font-semibold text-gray-700 flex items-center gap-1">
                            <span class="w-2 h-2 rounded-full bg-gray-900"></span> All Systems Operational
                        </p>
                    </div>
                </div>
            </div>
